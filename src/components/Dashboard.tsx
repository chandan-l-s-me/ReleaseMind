...
import { BASE_AGENTS } from "@/src/lib/agents";
...
function getRecommendationForRisk(risk: number) {
  if (risk > 70) return "Block Release";
  if (risk > 40) return "Canary Release";
  return "Full Release";
}
...
export default function Dashboard() {
      ...
      if (resolvedRepoUrl && resolvedRepoUrl.includes("github.com")) {
        ...
        setSourceSummary(
          githubContext.sourceType === "pull_request"
            ? `Fetched live GitHub PR context from ${githubContext.title} and read ${githubContext.includedFiles.length} repository files${githubContext.truncated ? " before hitting the analysis size limit" : ""}.`
            : `Fetched live GitHub repository context from ${githubContext.title} and read ${githubContext.includedFiles.length} repository files${githubContext.truncated ? " before hitting the analysis size limit" : ""}.`
        );
      }
  ...
  const generatePRReport = () => {
    if (!result) return;
    const currentRisk = Math.round(simulatedRisk ?? result.riskScore);
    const currentRecommendation = getRecommendationForRisk(currentRisk);
    const report = ` ReleaseMind Analysis Report
    
Risk Summary
- Risk Score: ${currentRisk}/100
- Recommendation: ${currentRecommendation}
- Confidence:${100 - currentRisk}%
...
Generated by ReleaseMind CI/CD Intelligence`;
  ...
  const currentRisk = Math.round(simulatedRisk ?? result?.riskScore ?? 0);
  const currentRecommendation = getRecommendationForRisk(currentRisk);
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/10">
                    ...
                    <span className={cn(
                      ...
                      currentRecommendation === "Block Release" ? "bg-red-500/10 text-red-500" :
                      currentRecommendation === "Canary Release" ? "bg-yellow-500/10 text-yellow-500" :
            ...
            <AnimatePresence mode="wait">
              {activeTab === "trends" ? (
                ...
                >
                  ...
                  {result.analysis.customAgentFindings && result.analysis.customAgentFindings.length > 0 && (
                    <div className="p-10 rounded-[40px] glass border-white/10">
                      <h3 className="text-white font-serif font-medium text-xl mb-10 flex items-center gap-3">
                      ...
                      <div className="space-y-4">
                        {result.analysis.customAgentFindings.map((item, index) => (
                          <div key={`${item.agent}-${index}`} className="p-6 rounded-3xl bg-white/5 border border-white/10">
                            <div className="text-[10px] font-bold text-accent uppercase tracking-widest mb-3">
                              {item.agent}
                            </div>
                  ...
                  <div className="p-10 rounded-[40px] glass border-white/10">
                    ...
                    <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
                      {BASE_AGENTS.map((agent) => {
                        const finding = result.analysis.baseAgentFindings?.find((item) => item.agent === agent.name);
                        return (
                          <div key={agent.name} className="p-6 rounded-3xl bg-white/5 border border-white/10">
                            <div className="text-[10px] font-bold text-accent uppercase tracking-widest mb-3">
                              {agent.name}
                            </div>
                            <p className="text-sm text-white/60 leading-relaxed font-light">
                              {finding?.finding || agent.description}
                            </p>
                          </div>
                        );
                      })}
        ...
        {showAgentSettings && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            >
              ...
              <div className="mb-8">
                <div className="text-[10px] font-bold text-white/30 uppercase tracking-widest mb-4">Core Agents</div>
                <div className="space-y-3">
                  {BASE_AGENTS.map((agent) => (
                    <div key={agent.name} className="p-4 rounded-2xl bg-white/[0.03] border border-white/10">
                      <div className="text-sm text-white font-medium mb-1">{agent.name}</div>
                      <div className="text-xs text-white/40 leading-relaxed font-light">{agent.description}</div>
                    </div>
                  ))}
                </div>
              </div>
